...
import { Filter, Target, Wallet, BarChart3 } from 'lucide-react';
          ...
          {trades.length === 0 && accounts.length === 0 ? (
               <div className="flex flex-col items-center justify-center py-20 border border-dashed border-white/10 rounded-3xl bg-white/[0.01]">
                  <div className="w-20 h-20 bg-brand-500/10 rounded-full flex items-center justify-center mb-6">
                      <Target size={40} className="text-brand-500" />
                  </div>
                  <h2 className="text-3xl font-bold text-white mb-2">Welcome to Yizinity, {user?.name}!</h2>
                  <p className="text-white/40 max-w-md text-center mb-8">
                      Your journal is currently empty. Start by adding a funded account or logging your first trade to see your analytics light up.
                  </p>
                  <div className="flex gap-4">
                      <button onClick={() => setActiveTab('funded')} className="px-6 py-3 bg-white/[0.05] hover:bg-white/[0.1] rounded-xl font-bold text-white transition-colors">
                          Add Account
                      </button>
                      <button onClick={() => setIsAddModalOpen(true)} className="px-6 py-3 bg-brand-500 hover:bg-brand-400 text-black rounded-xl font-bold transition-colors shadow-lg shadow-brand-500/20">
                          Log First Trade
                      </button>
                  </div>
               </div>
